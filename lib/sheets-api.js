...
class SheetsAPI {
    ...
    normalizeDate(dateString) {
        ...
        try {
            // "YYYY.MM.DD" 형식 처리 (예: "2025.11.28")
            const dotDatePattern = /^(\d{4})\.(\d{1,2})\.(\d{1,2})$/;
            const dotMatch = dateString.match(dotDatePattern);
            if (dotMatch) {
                const year = dotMatch[1];
                const month = String(dotMatch[2]).padStart(2, '0');
                const day = String(dotMatch[3]).padStart(2, '0');
                return `${year}-${month}-${day}`;
            }

            // "MM.DD" 형식 처리 (예: "11.28") - 현재 연도 사용
            const shortDatePattern = /^(\d{1,2})\.(\d{1,2})$/;
            const shortMatch = dateString.match(shortDatePattern);
            if (shortMatch) {
            ...
            // 일반적인 날짜 형식 처리 (Date 객체로 파싱 가능한 형식)
     ...
     * 편의 메서드: 오늘 날짜 데이터 가져오기
     */
    async getTodayData(spreadsheetIdOrUrl, range = 'Sheet1!A:Z', dateColumn = '날짜') {
        // URL이 전달된 경우 ID 추출
        let spreadsheetId = spreadsheetIdOrUrl;
        if (typeof spreadsheetIdOrUrl === 'string' && spreadsheetIdOrUrl.includes('/spreadsheets/d/')) {
            spreadsheetId = this.extractSpreadsheetId(spreadsheetIdOrUrl);
        }
        ...
        return this.filterByDate(parsedData, dateColumn, todayStr);
    }
}
