...
class DebuggerController {
    ...
    onEvent(source, method, params) {
        if (method === 'Page.javascriptDialogOpening') {
            ...
            // Tistory & Naver Blog "Restore draft" (임시저장) -> Cancel (false) to start fresh
            // "작성 중인 글이 있습니다. 복구하시겠습니까?" (Tistory)
            // "2025...에 저장된 글이 있습니다. 이어서 작성하시겠습니까?" (Naver)
            if (message.includes('임시저장') || message.includes('복구') || message.includes('restore') ||
                message.includes('저장된 글') || message.includes('이어서 작성')) {
                accept = false;
